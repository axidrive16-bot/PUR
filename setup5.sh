#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
# HalalScreen — Étape finale : Auth + App complète branchée
# ═══════════════════════════════════════════════════════════════════
set -e
echo "🕌 HalalScreen — Branchement complet..."

# ── Dépendances ───────────────────────────────────────────────────
npm install @supabase/supabase-js @supabase/ssr swr zustand immer
...
input, button, textarea { font-family: 'DM Sans', sans-serif !important; }
...
button:focus-visible, input:focus-visible { outline: 2px solid #00d68f; outline-offset: 2px; }
...
# ── src/components/App.tsx — App complète ─────────────────────────
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ user: data.user, needsConfirmation });
}
EOF

echo ""
echo "✅ App complète branchée !"
echo ""
echo "Lance : npm run dev"
echo "Ouvre : http://localhost:3000"
echo ""
echo "Fonctionnel :"
echo "  ✓ Recherche d'actions → /api/search"
echo "  ✓ Données FMP réelles → /api/stock/[ticker]"
echo "  ✓ Calcul AAOIFI serveur"
echo "  ✓ Auth Supabase (email)"
echo "  ✓ Portefeuille persisté (localStorage + futur Supabase)"
echo "  ✓ Watchlist persistée"
